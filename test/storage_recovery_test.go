...
func storageLostResults(t *testing.T) {
	...
	t.Run("untracked-sync-result", storageUntrackedResult)
}
...
func storageUntrackedResult(t *testing.T) {
	f := faultFixture(t)
	flag := filepath.Join(f.root, "fail-sync")
	stop := filepath.Join(f.root, "stop-escaped")
	script := filepath.Join(f.root, "runner.py")
	raw, err := os.ReadFile(script)
	if err != nil {
		t.Fatal(err)
	}
	needle := "start_new_session=True)\n sys.exit(0)"
	if !strings.Contains(string(raw), needle) {
		t.Fatal("missing escape fixture")
	}
	writeTest(t, script, []byte(strings.Replace(string(raw), needle, "start_new_session=True)\n open(inp['sync_fault'],'w').close()\n sys.exit(0)", 1)), 0700)
	a := submitOrder(t, f, map[string]any{"label": "untracked", "escape_check": true, "escape_stop": stop, "sync_fault": flag}, "untracked")
	storageSyncDaemon(t, f, flag)
	t.Cleanup(func() { writeTest(t, stop, nil, 0600) })
	awaitPaused(t, f, a["run_id"].(string))
	if err = os.Remove(flag); err != nil {
		t.Fatal(err)
	}
	got := f.await(t, a["run_id"].(string))
	if got["stage"] != "blocked:process_unknown" {
		t.Fatalf("untracked process evidence lost after storage recovery: %v", got)
	}
	result := got["steps"].([]any)[0].(map[string]any)["result"].(map[string]any)
	if result["untracked_processes"] != true {
		t.Fatalf("missing untracked evidence: %v", result)
	}
	if n := configCount(t, f, "SELECT count(*) FROM check_slots"); n != 1 {
		t.Fatalf("untracked check slot released: %d", n)
	}
	f.call(t, "submission", "cancel", a["submission_id"].(string), "--reason", "effects acknowledged but escaped process is live", "--acknowledge-effects")
	got = f.call(t, "run", "show", a["run_id"].(string), "--json")
	if got["stage"] != "blocked:process_unknown" {
		t.Fatalf("acknowledgement replaced stop evidence: %v", got)
	}
	if n := configCount(t, f, "SELECT count(*) FROM check_slots"); n != 1 {
