...
func (s *Store) failedExecution(x domain.Execution, out domain.Outcome, err error) {
	s.health.mu.Lock()
	defer s.health.mu.Unlock()
	s.pauseStorageLocked(err)
	if s.health.pending == nil {
		s.health.pending = make(map[string]failedStep)
	}
	s.health.pending[x.StepID] = failedStep{execution: x, outcome: out}
}
...
func (s *Store) StorageReady(ctx context.Context) bool {
	...
	pending := make([]failedStep, 0, len(s.health.pending))
	...
	for _, failed := range pending {
		...
		out := domain.Outcome{Kind: "interrupted", ExitCode: -1, Untracked: failed.outcome.Untracked}
		if out.Untracked || !stopped && owner.PID != 0 {
